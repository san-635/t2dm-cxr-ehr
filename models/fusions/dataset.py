...
class EHR_ECG_CXR_Dataset(Dataset):
    ...
    def get_text(self, index):
        ...
        return torch.tensor(ehr_data, dtype=torch.float32), torch.tensor(t_mask, dtype=torch.float32), len(t_mask), ecg_data, e_mask, len(e_mask), label, cxr_path
    ...
    def __getitem__(self, index):
        ehr_data, t_mask, t_len, ecg_data, e_mask, e_len, label, cxr_path = self.get_text(index)
        ...
        return {
            ...
            "t_len": t_len,         # a scalar (i.e., no. of bins)
            "ecg": ecg_data,        # a 2D tensor (shape: max_ecg_len-1, ecg_n_var)
            "e_mask": e_mask,       # a 1D tensor (shape: max_ecg_len-1)
            "e_len": e_len,         # a scalar (i.e., max_ecg_len-1)
    ...
    def my_collate(self, batch):
        # RE-STUCTURE BATCH #
        filtered_batch = [sample for sample in batch if sample['t_len']>0]
        batch_size = len(filtered_batch)
        keys = set([key for sample in filtered_batch for key in sample.keys()])
        dict_batch = {key: [sample[key] for sample in filtered_batch] for key in keys}
        ...
        # dict_batch["e_len"] and batch["label"] remain unchanged
