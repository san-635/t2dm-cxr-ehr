...
# relevant to this study (loss computation and metrics)
def compute_ehr_ecg_cxr(pl_module, batch):
    phase = "train" if pl_module.training else "val"

    # call infer function from vilt_model.py, returns ret dict with text_feats, image_feats, cls_feats, etc.
    infer = pl_module.infer(batch, mask_text=False, mask_image=False)

    imgcls_logits = pl_module.ehr_ecg_cxr_classifier(infer["cls_feats"])                        # pass cls_feats to the binary classifier
    imgcls_labels = torch.tensor(batch["label"]).to(pl_module.device).long()
    if imgcls_logits.size(-1)>1:
        imgcls_loss = F.cross_entropy(imgcls_logits, imgcls_labels)                             # compute cross entropy loss (performs softmax internally)
    else:
        imgcls_loss = F.binary_cross_entropy_with_logits(imgcls_logits.squeeze(-1), imgcls_labels.float())          # compute binary cross entropy loss (performs sigmoid internally)

    ret = {"ehr_ecg_cxr_loss": imgcls_loss, "ehr_ecg_cxr_logits": imgcls_logits, "ehr_ecg_cxr_labels": imgcls_labels}

    # getattr returns a Metric object, and its __call__() is called:
        # this calls update() with the arguments in the second set of parentheses
        # and finally returns the metric upto the current step by calling compute()
    loss = getattr(pl_module, f"{phase}_ehr_ecg_cxr_loss")(ret["ehr_ecg_cxr_loss"])
    acc = getattr(pl_module, f"{phase}_ehr_ecg_cxr_accuracy")(ret["ehr_ecg_cxr_logits"], ret["ehr_ecg_cxr_labels"])
    auroc = getattr(pl_module, f"{phase}_ehr_ecg_cxr_AUROC")(ret["ehr_ecg_cxr_logits"], ret["ehr_ecg_cxr_labels"])
    auprc = getattr(pl_module, f"{phase}_ehr_ecg_cxr_AUPRC")(ret["ehr_ecg_cxr_logits"], ret["ehr_ecg_cxr_labels"])
    
    if pl_module.hparams.config["test_only"] == False:
        pl_module.log(f"ehr_ecg_cxr/{phase}/loss_step_upd", loss)                                   # overall loss upto current step; for progress bar
...
def compute_hatememes(pl_module, batch):
    ...
    ret = {"hatememes_loss": imgcls_loss, "hatememes_logits": imgcls_logits, "hatememes_labels": imgcls_labels}

    loss = getattr(pl_module, f"{phase}_hatememes_loss")(ret["hatememes_loss"])
    acc = getattr(pl_module, f"{phase}_hatememes_accuracy")(ret["hatememes_logits"], ret["hatememes_labels"])
    auroc = getattr(pl_module, f"{phase}_hatememes_AUROC")(ret["hatememes_logits"], ret["hatememes_labels"])        
...
# relevant to this study (initialise weights for different blocks/layers of transformer)
