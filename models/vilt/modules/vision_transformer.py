...
class Attention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            mask = mask.bool()
            mask_inv = ~mask[:, None, None, :]
            attn = attn.masked_fill(mask_inv, float("-inf"))
...
class VisionTransformer(nn.Module):
    ...
    def visual_embed(self, _x, max_image_len=200, mask_it=False):
        ...
        select = select.to(patch_index.device)
    ...
    def visual_embed_for_missing(self, batch, max_image_len=200):
        B = batch.shape[0]
        cls_token = self.cls_token.expand(B, -1, -1)
        x_list, x_mask_list, pos_embed_list = [], [], []

        for i in range(B):
            if (batch[i].sum(dim=(0, 1, 2)) != 0):
                x = self.patch_embed(batch[i].unsqueeze(0))
                x_mask = (batch[i].sum(dim=0) != 0).float()[None, None, :, :]
                x_mask = F.interpolate(x_mask, size=(x.shape[2], x.shape[3])).long()
                x_h = x_mask[:, 0].sum(dim=1)[:, 0]
                x_w = x_mask[:, 0].sum(dim=2)[:, 0]

                _, C, H, W = x.shape
                spatial_pos = (self.pos_embed[:, 1:, :].transpose(1, 2).view(1, C, self.patch_dim, self.patch_dim))
                pos_embed = torch.cat(
                    [
                        F.pad(
                            F.interpolate(
                ...
                if (max_image_len < 0 or max_image_len is None or not isinstance(max_image_len, int)):
                    eff = x_h * x_w
                    max_image_tokens = eff.max()
                else:
                    eff = x_h * x_w
                    max_image_tokens = min(eff.max(), max_image_len)
                ...
                valid_nums = [v.size(0) for v in valid_row_idx]
                non_valid_nums = [v.size(0) for v in non_valid_row_idx]
                pad_nums = [max_image_tokens - v for v in valid_nums]

                select = list()
                for j, (v, nv, p) in enumerate(zip(valid_nums, non_valid_nums, pad_nums)):
                    if p <= 0:
                        valid_choice = torch.multinomial(torch.ones(v).float(), max_image_tokens)
                        select.append(valid_row_idx[j][valid_choice])
                    else:
                        pad_choice = torch.multinomial(torch.ones(nv).float(), p, replacement=True)
                        select.append(torch.cat([valid_row_idx[j], non_valid_row_idx[j][pad_choice]], dim=0))

                select = torch.cat(select, dim=0)
                x = x[select[:, 0], select[:, 1]].view(1, -1, C).squeeze(0)
                x_mask = x_mask[select[:, 0], select[:, 1]].view(1, -1).squeeze(0)
                pos_embed = pos_embed[select[:, 0], select[:, 1]].view(1, -1, C).squeeze(0)

                x_list.append(x)
                x_mask_list.append(x_mask)
                pos_embed_list.append(pos_embed)

            # zero-filled tensor as image (added for this study's "missing CXR modality" ablation study)
            else:
                if (max_image_len < 0 or max_image_len is None or not isinstance(max_image_len, int)):
                    max_image_tokens_missing = 1
                x_list.append(torch.zeros((max_image_tokens_missing, self.embed_dim), device=batch.device))
                x_mask_list.append(torch.ones((max_image_tokens_missing), device=batch.device))
                pos_embed_list.append(torch.zeros((max_image_tokens_missing, self.embed_dim), device=batch.device))
        
        max_tokens = max([x.size(0) for x in x_list])
        x_list = [F.pad(x, (0, 0, 0, max_tokens - x.size(0))) if x.size(0) < max_tokens else x for x in x_list]
        x_mask_list = [F.pad(x_mask, (0, max_tokens - x_mask.size(0))) if x_mask.size(0) < max_tokens else x_mask for x_mask in x_mask_list]
        pos_embed_list = [F.pad(pos_embed, (0, 0, 0, max_tokens - pos_embed.size(0))) if pos_embed.size(0) < max_tokens else pos_embed for pos_embed in pos_embed_list]

        x = torch.stack(x_list)
        x_mask = torch.stack(x_mask_list)
        pos_embed = torch.stack(pos_embed_list)

        x_mask = torch.cat([torch.ones(x_mask.shape[0], 1).to(x_mask), x_mask], dim=1)
        x = torch.cat((cls_token, x), dim=1)
        pos_embed = torch.cat((self.pos_embed[:, 0, :][:, None, :].expand(B, -1, -1), pos_embed), dim=1)
        x = x + pos_embed
        x = self.pos_drop(x)
...
@register_model
def vit_base_patch32_384(pretrained=False, **kwargs):   # this is the model used
